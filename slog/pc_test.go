...
func check(t *testing.T, testname string, buf *PrintCtx, s string) {
	bytes := buf.Bytes()
	str := buf.String()
...
func fillString(t *testing.T, testname string, buf *PrintCtx, s string, n int, fus string) string {
...
// The initial contents of buf corresponds to the string s;
// the result is the final contents of buf returned as a string.
func fillBytes(t *testing.T, testname string, buf *PrintCtx, s string, n int, fub []byte) string {
	check(t, testname+" (fill 1)", buf, s)
	for ; n > 0; n-- {
		m, err := buf.Write(fub)
		if m != len(fub) {
...
func TestNewBufferString(t *testing.T) {
	buf := NewPrintCtxString(testString)
	check(t, "NewBufferString", buf, testString)
}

// Empty buf through repeated reads into fub.
// The initial contents of buf corresponds to the string s.
func empty(t *testing.T, testname string, buf *PrintCtx, s string, fub []byte) {
...
func TestReadFromNegativeReader(t *testing.T) {
	...
	defer func() {
		...
		case nil:
			t.Fatal("logg/slog.PrintCtx.ReadFrom didn't panic")
		case error:
			// this is the error string of errNegativeRead
			wantError := "logg/slog.PrintCtx: reader returned negative count from Read"
...
func TestWriteAppend(t *testing.T) {
	...
	if !bytes.Equal(got.Bytes(), want) {
		t.Fatalf("Bytes() = %q, want %q", got.String(), want)
	}
...
func TestReadBytes(t *testing.T) {
	for _, test := range readBytesTests {
		buf := NewPrintCtxString(test.buffer)
		var err error
